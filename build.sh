...
mysql_password="slave" # 主服务器允许从服务器登录的密码
...
#################### 主服务器操作 ####################开始
# 修改加密插件(8版本默认sha加密)
pwd_plugin_alter='ALTER USER "'$mysql_user'"@"%" IDENTIFIED WITH "mysql_native_password";
ALTER USER "'$mysql_user'"@"%" IDENTIFIED BY "'$mysql_password'";'

# 在主服务器上添加数据库用户
priv_stmt='GRANT REPLICATION SLAVE ON *.* TO "'$mysql_user'"@"%"; FLUSH PRIVILEGES;'

docker exec $master_container sh -c "export MYSQL_PWD='$root_password'; mysql -u root -e '$pwd_plugin_alter'"
...
for slave in "${slave_containers[@]}";do
  # 修改加密插件
  docker exec $slave sh -c "export MYSQL_PWD='$root_password'; mysql -u root -e '$pwd_plugin_alter'"
  # 从服务器连接主互通
